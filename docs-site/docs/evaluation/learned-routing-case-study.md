---
title: "Case study: learned upstream selection"
...
[workload-to-routing guide](../routing/learned-routing-policy.md#a-complete-workload-to-routing-scenario)
for commands and the [caller-facing explanation](../routing/learned-routing-policy.md)
...
The original internal index contains protected artifact locations and is not a
...
## Scenario design before collecting data

The deployment team first defines the jobs the routing group must complete,
then chooses candidates to test against those jobs. In a real deployment, cover
representative request sizes, language, system instructions, output constraints,
client dialects, tools and session histories. Define success before looking at
model scores: exact results, extraction correctness, unit tests or reviewer-recorded
acceptance can establish whether a cheaper model is sufficient.

Choose an anchor for the comparison bar and validate exact upstream model/API
combinations directly and through the router. Collect each approved workload
against every candidate through isolated single-target calibration groups. Keep
failed responses and unknown costs visible; they must not disappear from training
or evaluation. Use objective verifiers first and approved third-party judging
when no verifier is available.

```mermaid
flowchart TD
  A[Define workloads and acceptance tests] --> B[Validate upstream candidates]
  B --> C[Collect approved dataset and candidate outcomes]
  C --> D[Build shared features]
  D --> E[Train quality and output-token models]
  E --> F[Calibrate and evaluate held-out sessions]
  F --> G[Versioned bundle]
  G --> H[Shadow and inspect recommendations]
  H --> I[Enforce only after workload gates pass]
```

At inference time the router applies access and request-shape eligibility,
then LRP scores the remaining targets with the trained bundle. The service
returns the lowest estimated-cost target predicted to meet the configured floor.
The router calls that upstream and records actual usage, latency and fallback
separately from the prediction. Training and judging do not run in that request
path. This case study demonstrates that sequence and the evidence needed to
...
| Input tokens / request bytes | Threads | Embedding p99 | Full features p99 | Embedding ≤15 ms / features under 40 ms |
|---|---:|---:|---:|---|
...
[deadline configuration guidance](../routing/learned-routing-policy.md#configurable-wait-for-latency-tolerant-workloads-and-tests).
